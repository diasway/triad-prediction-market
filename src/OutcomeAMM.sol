...
contract OutcomeAMM is IERC1155Receiver, ReentrancyGuard, AccessControl {
    ...
    event LiquidityAdded(
        address indexed provider, uint256 yesAmount, uint256 noAmount, uint256 lpMinted
    );
    event LiquidityRemoved(
        address indexed provider, uint256 yesAmount, uint256 noAmount, uint256 lpBurned
    );
    event Swap(
        address indexed trader, uint256 indexed tokenIn, uint256 amountIn, uint256 amountOut
    );
    ...
    constructor(
        ...
        require(
            admin != address(0) && collateral_ != address(0) && outcomeToken_ != address(0), "ZERO"
        );
